...
const htmlHandler = () => {
  return gulp.src('./src/html/*.html') // 找到要压缩的 html 文件
...
const htmlHandler1 = () => {
  return gulp.src('./src/index.html') // 找到要压缩的 html 文件
             .pipe(htmlmin({ // 想进行压缩, 需要在这个对象里面进行配置
               removeAttributeQuotes: true, // 移出属性上的双引号
               removeComments: true, // 移除注释
               collapseBooleanAttributes: true, // 把值为布尔值的属性简写
...
const htmlHandler2 = () => {
  return gulp.src('./src/login.html') // 找到要压缩的 html 文件
             .pipe(htmlmin({ // 想进行压缩, 需要在这个对象里面进行配置
               removeAttributeQuotes: true, // 移出属性上的双引号
               removeComments: true, // 移除注释
               collapseBooleanAttributes: true, // 把值为布尔值的属性简写
...
const watchHandler = () => {
  ...
  gulp.watch('./src/index.html', htmlHandler1)
  gulp.watch('./src/login.html', htmlHandler2)
  gulp.watch('./src/lib/**', libHandler)
  gulp.watch('./src/images/**', imgHandler)
}
module.exports.default = gulp.series(
  delHandler,
  gulp.parallel(cssHandler, jsHandler, htmlHandler,htmlHandler1,htmlHandler2, imgHandler, libHandler),
  // serverHandler,
  watchHandler
)